#!/usr/bin/env python3
"""Convert the ten original PSX camera background TIMs to Wii U RLE textures."""
...
def parse_tim(path: Path) -> tuple[int, int, list[int], bytes]:
    ...
    mode = flags & 0x7
    if mode not in (0, 1):
        raise ValueError(f"{path}: only indexed 4-bit/8-bit TIMs are supported")
    if (flags & 0x8) == 0:
...
def encode_rows(width: int, height: int, pixels: bytes) -> tuple[list[int], bytes]:
    ...
    if len(encoded) > 0xFFFF:
        raise ValueError(
            f"RLE stream is {len(encoded)} bytes; TextureRle row offsets are 16-bit"
        )
...
def emit_texture(source: list[str], camera: int, width: int, height: int,
    ...
    symbol = f"gPsxCamera{camera:02d}Texture"
    unique = f"kPsxCamera{camera:02d}"
    offsets, runs = encode_rows(width, height, pixels)

    palette_size = 16 if max(pixels, default=0) < 16 else 256
    palette = [psx_colour_to_rgba(value) for value in colours[:palette_size]]
...
def main() -> None:
    ...
    source = [
        "/* Generated from the ten original PSX camera background TIMs. */\n",
    ...
    for camera in range(1, 11):
        path = args.map_root / f"{camera:02d}" / "CAM.TIM"
        ...
        print(
            f"camera background {camera:02d}: {path} -> "
            f"{width}x{height}, {len(set(pixels))} indices"
        )

    source.append("const TextureRle *const gPsxCameraTextures[10] = {\n")
    source.extend(f"    &{symbol},\n" for symbol in symbols)
    source.append("};\n")
    header.append("\nextern const TextureRle *const gPsxCameraTextures[10];\n")
